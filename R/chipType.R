...
#' where the changes are symmetrical between two biological
#' conditions (unimodel on fold changes); or the changes might be
#' globally one-side accompanied with some non-changing bindings (bimodel
#' on fold changes). This function help to determine the binding type of
#' given ChIP-Seq samples from two biological conditions using kernal
#' density information of raw fold changes. As this function
#' was designed to work on the raw counts (no normalization needed),
#' only one replicate from each condition is allowed (input a two-column
#' count matrix); otherwise, coverage difference acorss replicates
...
#' In detail, if top two estimated modes on smoothed kernal density have
...
#' @param h Initial smoothing factor when estimating kernal density of
#' raw fold changes for bump hunting. (Default: 0.1)
#' @param plot A logical indicator that if M-A plot and smoothed kernal
#' density should be visualized. (Default: TRUE)
#'
#' @importFrom matrixStats rowMaxs
#' @importFrom matrixStats rowDiffs
#'
#' @return
#' A character with value either "bimodel" or "unimodel" to represent
#' estimated binding type.
#'
#' @export
#'
#' @examples
